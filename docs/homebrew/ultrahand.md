...
- The latest release of [Ultrahand](https://github.com/ppkantorski/Ultrahand-Overlayreleases/) (the `sdout.zip` file)

## Installation instructions:
1. Boot into hekate and go to `Tools` > `USB Tools` > `SD Card`, then plug your Switch into your PC via USB.
1. Your microSD card should now be accessible on your PC, open it.
1. Extract the Ultrahand `.zip` file to a location on your computer.
...
1. Copy the *contents* of the (extracted) `.zip` file to the root of your microSD card, then copy `ovlmenu.ovl` to `sd:/switch/.overlays`.
    - **Optional:** You can verify if you've installed Ultrahand correctly, you should have a folder called `420000000007E51A` (nx-ovlloader) in `sd:/atmosphere/contents` and the `ovlmenu.ovl` (Ultrahand) file in `sd:/switch/.overlays`.
1. Boot into CFW.
...
- [KeyX](https://github.com/TOM-BadEN/KeyX) (this requires you to also install the KeyX sysmodule)

## Troubleshooting
### My Switch crashes on boot after I installed Ultrahand!:

**Cause:** If your Switch crashes with Error code `2001-0123 (0xf601)` and Title ID `420000000007E51A`, you didn't successfully install Ultrahand or you aren't using the latest release of Ultrahand, re-follow the installation instructions above.

### My Switch crashes when I open an overlay via Ultrahand!:
